        ...
        if url or url and bypass_cache:
            if self.is_fa:
                ...
                while True:
                    ...
                    if next_btn is None:
                        logger.success(
                            f"{p} pages found! Along with the total of {aw} artworks counted")
                        ...
                        self.update_json(
                            cache_filename, save_to_cache, time_series=False)
                        break
                return
            ...
            if self.is_ws:
                ...
                p, aw = self.page_metadata.get(
                    'pages'), self.page_metadata.get('artworks')
...
class SubmissionParser(Parinton):
    def __init__(self, url: str, json_fn: Optional[str] = None):
        ...
        if url:
            if self.is_fa:
                self._art_page = self.soup_req(
                    url, self.get_ua(self.b_furaffinity))
            ...
            if self.is_ib:
                self._art_page = self.soup_req(
                    url, self.get_ua(self.b_inkbunny))

            self._fa_contents: Tag = self._art_page.select_one(
                ".submission-content section")
            self._date: datetime | str | None = None

            # We pass dates here, so we can calculate the difference here for the updater
            if self.is_fa:
                fa_date = self._fa_contents.select_one(
                    "span.popup_date")['title']
    ...
    def parse(self, include_retrieved_item: bool = False):
        data = {
            "title": self.title,
    ...
    def title(self):
        if self.is_fa:
            fa_title = self._fa_contents.find(
                class_="submission-title").text.strip()
            return fa_title
    ...
    def tags(self):
        ...
        if self.is_fa:
            tags_iterable: Union[ResultSet[Tag], Any] = self._art_page.select(
                "section.tags-row span.tags")
